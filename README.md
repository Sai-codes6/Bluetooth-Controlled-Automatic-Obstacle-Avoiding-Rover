...
A dual-mode Arduino Nano rover I drive from an Android app over Bluetooth —
with a hard safety override that won't let me crash it. The moment an obstacle appears
within 50 cm ahead, the rover ignores my commands, reverses to safety, and waits for a
clear path before listening again.
...
│   └── layout-photo.jpeg           # Physical build reference
└── app/
...
1. Power the rover and pair the phone with `HC-05` (PIN `1234`/`0000`).
...
The override reverses by **time**, not encoders, so it needs to know the rover's actual
...
Because the safety reverse always uses the fixed `BACKUP_SPEED` (not the live 0–9 speed
setting), this calibration stays accurate no matter how fast the rover is driven.
